...
2. **每个 change 的完整生命周期由一个 myspec agent 执行**。从设计到归档，全程在同一个 agent 中完成
3. **答复团队作为子智能体直接与 myspec agent 交互**。主会话不充当中间人
4. **主会话仅在合并时机上介入**。协调多个 change 对 main 的串行合入
...
│  - 不进入设计/审查/验证决策       │
└────┬─────────────────────────────┘
     │ 派遣（每个 change 一个 myspec agent）
     │
┌────▼─────────────────────────────────────────┐
│  myspec agent（change 生命周期负责人）          │
│  在 worktree 中执行完整 myspec 流程            │
│                                               │
│  设计阶段 ──► 方案阶段 ──► 实施阶段            │
│      │            │           │               │
│      │            │           ▼               │
...
- **职责**：
  - 创建 worktree（myspec-gwt）
  - 执行设计阶段的苏格拉底式对话（myspec-br）
  - 生成方案工件（myspec-propose）
  - 实施代码（myspec-apply）
  - 协调审查和验证（派遣答复团队子智能体）
  - 执行合并和归档（myspec-merge）
- **权限**：worktree 中的所有文件读写
- **在需要评估/决策时，派遣答复团队子智能体**

### 答复团队 — 子智能体（由 myspec agent 按需派遣）

答复团队是 myspec agent 的"参谋部"。myspec agent 在关键节点派遣他们进行评估、决策、复核，他们完成后将结论返回给 myspec agent。
...
```
Phase 1: 设计（myspec-br 苏格拉底式对话）
  myspec agent 执行 myspec-br 技能
  │
  ├── 需要产品决策？→ 派遣 pm 子智能体评估 → 接收结论
  ├── 需要技术决策？→ 派遣 architect 子智能体评估 → 接收结论
  ├── 综合结论继续苏格拉底式对话
  └── 设计文档完成，进入下一阶段

Phase 2: 方案（myspec-propose）
  自动生成 proposal / specs / design / tasks 工件

Phase 3: 实施（myspec-apply）
  在 worktree 中按 task group 编码，每组完成后提交

Phase 4: 审查
  ├── 派遣 reviewer 子智能体审查代码
  ├── 接收审查报告
  └── 修复问题，重新派遣 reviewer 直到通过

Phase 5: 验证（myspec-verify）
  ├── 派遣 qa 子智能体验证功能
  ├── 接收验证报告
  └── 回填工件使其匹配最终实现

Phase 6: 合并（myspec-merge）
  ├── 通知主会话"就绪待合入"
  ├── 等待主会话调度合并时机
  ├── 主会话确认后执行 myspec-catchup（同步 main）
  └── 执行 myspec-merge（合入 main）
...
- **主会话调度**：主会话根据当前 main 状态和其他 change 的进度，决定合入顺序
...
派遣 myspec agent 时，prompt 必须包含：
1. **问题描述** — 用户提出的问题或需求
2. **全局背景** — 整个系统在做什么，当前功能的完整上下文
3. **change 名称** — kebab-case 标识符
4. **验收标准** — 如何判断这个 change 完成

### myspec agent 派遣答复团队

派遣答复团队子智能体时，prompt 必须包含：
1. **具体问题** — 需要评估/决策/审查的具体内容
2. **全局上下文** — 当前 change 在做什么，系统全貌
3. **主动发现要求** — 明确要求输出"发现的其他问题"
4. **用户视角** — 提醒以真实用户身份思考

### 并行限制

- 主会话同时管理的 myspec agent 不超过 3 个
