...
- **派遣时机**：设计阶段的用户体验评估、交互方案决策
- **职责**：以真实用户视角评估方案、发现体验痛点、质疑不合理假设
- **主动发现**：
  - 这个操作对**真实用户**来说直觉吗？需要几步完成？
  - 有没有**更简单的方式**达成同样的目标？
  - 用户**会犯什么错**？系统有没有兜底？
  - 文案和提示**用户能看懂**吗？
...
```
Phase 1: 设计
  Skill: myspec-br（子智能体团队间的苏格拉底式对话）
  │
  ├── myspec agent 按需派遣多个视角的子智能体（pm、architect、用户代表等）
  ├── 子智能体之间互相挑战假设、评估方案、质疑盲点
  ├── myspec agent 综合各视角结论，推进设计收敛
  ├── myspec agent 自行决定需要哪些视角、派遣多少子智能体
  ├── 设计文档完成
  └── 自动调用 Skill("myspec-gwt") 创建工作树（默认执行，无需用户确认），后续阶段在工作树中进行
...
5. **答复团队自派指引** — 告知 myspec agent 它拥有 Agent 工具，可按需自行派遣以下子智能体：
   ...
   - `user-advocate` — 用户体验评估、交互方案决策、以真实用户视角质疑假设
   - `pm` — 产品需求分析、用户路径评估
   - `reviewer` — 代码审查（安全/性能/一致性/用户路径）
   - `qa` — 功能验证（真实用户流程、错误场景、查看路径）
   - 用完即释放，不常驻
6. **主动发现要求** — 答复团队子智能体必须主动发现被遗漏的问题，不只回答被问到的
7. **设计自主权** — 设计阶段的苏格拉底式对话完全由 myspec agent 通过派遣子智能体团队完成，不得将设计问题抛回主会话。myspec agent 自行决定派遣哪些视角的子智能体（pm、architect、user-advocate 等），通过多视角互相挑战来收敛设计
